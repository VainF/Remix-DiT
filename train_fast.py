...
def main(args):
    ...
    if rank == 0:
        ...
        experiment_dir = f"{args.results_dir}/{args.prefix}"  # Create an experiment folder
    ...
    for name, param in model.named_parameters():
        if 'coefficients' in name:
            ...
            model_prams.append(param)

    opt = torch.optim.AdamW(
        [
            {"params": model_prams, "lr": lr},
            {"params": coeff_params, "lr": 1e-2}
        ],
        lr=lr,
        weight_decay=0.0
    )
    sched = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=args.epochs * len(loader), eta_min=0.5*lr)
        ...
        for x, y in loader:
            ...
            if train_steps % args.log_every == 0:
                ...
                if rank == 0:
                    ...
                    wandb.log({"train_loss": avg_loss, "misc/steps_per_sec": steps_per_sec, "misc/lr": opt.param_groups[0]['lr'], "coeffs": wandb.Image(coeffs)}, step=train_steps)
                    #except:
                    #    pass
